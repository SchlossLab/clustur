...
```{r}
distance_data <- read_dist(example_path("amazon_column.dist"), count_table, cutoff = 0.03)
```

```{r}
distance_data <- read_dist(example_path("amazon_phylip.dist"), count_table, cutoff = 0.03)
```
...
```{r}
count_table <- read_count(example_plath("amazon.tabular.count_table")) #should be a tsv or mothur-native count file
```

```{r}
count_table <- read_count(example_plath("amazon.compressed.count_table")) #should be a tsv or mothur-native count file
```
...
```{r}
cluster_data <- cluster(distance_data, count_table, cutoff=0.03, method = "opticlust") # this needs to make sure that dist_data and count_table are compatible
```

### Selecting different clustering methods

```{r}
cluster_data <- cluster(distances, count_table, cutoff=0.03, method = "furthest")
cluster_data <- cluster(distances, count_table, cutoff=0.03, method = "nearest")
cluster_data <- cluster(distances, count_table, cutoff=0.10, method = "average")
cluster_data <- cluster(distances, count_table, cutoff=0.03, method = "weighted")
```
...
clusters <- cluster(distances, count_table, cutoff=0.03, method = "opticlust")

get_cutoff(cluster_data)
get_clusters(cluster_data)
get_shared(cluster_data)
