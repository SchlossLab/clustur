---
title: "clustur" output: rmarkdown::html_vignette vignette: >
%\VignetteIndexEntry{clustur} %\VignetteEngine{knitr::rmarkdown}
editor_options: 
...
all(full_count_table == sparse_count_table)
```
...
frame version of the distances, you can use `get_distances(distance_data)`.

```{r}
all(get_distance_data_frame(column_distance) == get_distance_data_frame(phylip_distance))
```
...
```{r}
cluster_data <- cluster(column_distance)
```


### Selecting different clustering methods
```{r}
cluster_data <- cluster(column_distance, method = "furthest")
cluster_data <- cluster(column_distance, method = "nearest")
cluster_data <- cluster(column_distance, method = "average")
cluster_data <- cluster(column_distance, method = "weighted")
```
...
clusters <- cluster(column_distance)
get_label(clusters)
