...
#ifndef REFACTOR_LISTVECTOR_H
#define REFACTOR_LISTVECTOR_H
...
public:
    ...
    explicit ListVector(const int n): data(n, "") ,
    ...
    [[nodiscard]] int getNumBins() const { return numBins;}
    [[nodiscard]] int getNumSeqs() const { return numSeqs;}
    [[nodiscard]] int getMaxRank() const { return maxRank;}
    [[nodiscard]] std::string getLabel() const { return label; }

    [[nodiscard]] std::string get(long long) const;
