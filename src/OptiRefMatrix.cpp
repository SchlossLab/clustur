...
bool OptiRefMatrix::isCloseFit(const long long i, const long long toFind, bool& isFit){
    ...
    if (!isRef[toFind]) { //are you a fit seq
        found = std::binary_search(closeness[i].cbegin(), closeness[i].cend(), toFind);
...
/***********************************************************************/
std::vector<long long> OptiRefMatrix::getCloseFitSeqs(long long index){
    std::vector<long long> closeSeqs;
    ...
    if (index > closeness.size()) { Rcpp::stop("[ERROR]: index is not valid.\n"); } //
        ...
        //reference seqs all have indexes less than refEnd
    for (const long long it : closeness[index]) {
        if (!isRef[it]) closeSeqs.emplace_back(it);  //you are a fit seq
    }

    return closeSeqs;
}
/***********************************************************************/
std::vector<long long> OptiRefMatrix::getCloseRefSeqs(long long index) {
    std::vector<long long> closeSeqs;
    ...
    for (long long it : closeness[index]) {
        if (isRef[it]) { closeSeqs.emplace_back(it); } //you are a ref seq
    }
    return closeSeqs;
}
