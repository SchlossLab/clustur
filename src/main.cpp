...
std::string ClusterClassic(const std::vector<int> &xPosition,
    ...
    ReadPhylipMatrix reader(filePath, cutoff);
    reader.read();
    auto *distanceMatrix = reader.getDMatrix();
    auto *list = reader.getListVector();
    auto *cluster = new CompleteLinkage(new RAbundVector(), list,
                                      distanceMatrix, 0.2, "furthest", -1.0);
    ClusterCommand command;
    return command.runMothurCluster(cluster, distanceMatrix, cutoff, list);
}

void CreateRandonData(std::vector<int> &xPositions, std::vector<int> &yPositions, std::vector<double> &data,
                      const int amount) {
...
//
//     CreateRandonData(xPosition, yPosition,data, 10000);
